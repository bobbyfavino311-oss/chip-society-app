/**
 * ViceBackground — Luxury synthwave poker room.
 * Dark navy sky with scattered stars, large pink flamingo (left),
 * dark navy palm (right), thin cyan horizon lines. Nothing more.
 */
...
// Normalized: feet at (0,0), head rises to about (11, -55)
// Multiply all coords by `s`, add (ox, oy) for position
function NeonFlamingo({ ox, oy, s = 1, color = '#FF2FAE', op = 1 }: {
  ...
  const sw = 2.0 * Math.min(s, 2.5);   // stroke doesn't need to scale as aggressively
  ...
  return (
    <View style={StyleSheet.absoluteFillObject} pointerEvents="none">
      <Svg width={W} height={H} viewBox={`0 0 ${VW} ${VH}`} preserveAspectRatio="xMidYMid meet">
        ...
        <Rect x={0} y={0} width={VW} height={VH} fill="url(#vbPurple)" />
        <Rect x={0} y={0} width={VW} height={VH} fill="url(#vbPinkFoot)" />
        ...
        {/* feet at (72, VH), scale=2.3 → head at ~(72+25, VH-125) ≈ (97, 719) */}
        <NeonFlamingo ox={72} oy={VH} s={2.3} color="#FF2FAE" op={0.55} />

        {/* ── Dark navy palm tree — bottom-right (background accent) ───── */}
